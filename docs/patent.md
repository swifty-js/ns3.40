...
[0047] 图 1 是本发明实施例提供的拥塞控制方法整体流程图。
...
[0056] 本实施例提供一种实现实施例一所述方法的拥塞控制系统，包括状态采集模块、决策模块、决策应用模块和通信模块。
