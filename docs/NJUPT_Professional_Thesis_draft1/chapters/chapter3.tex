...
本章详细介绍Lark算法的设计与实现，包括系统架构概述、强化学习问题建模、15维观测空间设计、多信号融合拥塞检测算法、差异化拥塞响应机制、强化学习驱动的自适应参数调优、融合控制策略以及每流独立状态管理机制。
...
% 网络仿真模块
\node[module, fill=blue!12] (sim) at (0,0) {\textbf{网络仿真模块}\\(ns-3 + MTP)};
...
% 环境模块（底部跨越）
\node[module, fill=orange!12, minimum width=13cm] (env) at (5.5, -4.5) {\textbf{拥塞控制环境模块}（OpenGym / TcpLarkEnv）——15维观测空间采集 + 动作应用 + 奖励计算};

% 箭头
\draw[arrow, blue!70] (sim3.east) -- ++(1.0,0) |- node[above, font=\scriptsize, pos=0.25] {15维观测} (zmq.west);
...
\textbf{网络仿真模块}基于ns-3（Network Simulator 3）离散事件仿真平台构建，实现了完整的TCP/IP协议栈。ns-3是目前学术界和工业界广泛使用的网络仿真工具，提供了丰富的网络协议模型和灵活的可扩展架构。网络仿真模块负责构建网络拓扑（如Leaf-Spine拓扑、哑铃型拓扑等）、模拟数据包的传输过程、执行离散事件驱动的仿真调度。

为了提高仿真效率，网络仿真模块采用了多线程并行化架构（MTP-Interface）。传统的ns-3采用单线程串行架构，在大规模网络拓扑和长时间仿真场景下效率较低。MTP-Interface将网络拓扑划分为多个逻辑分区，在多个线程上并行执行各分区的仿真事件，通过保守同步协议（Conservative Synchronization Protocol）协调线程间的事件同步。

\textbf{拥塞控制环境模块}基于OpenGym框架实现，负责在TCP协议栈与强化学习智能体之间建立桥梁。OpenGym是ns3-gym项目提供的强化学习环境接口，遵循OpenAI Gym的标准API规范。拥塞控制环境模块在TCP协议栈的关键回调点采集状态参数，构建15维观测空间，并将智能体的决策动作应用于TCP连接。
...
\textbf{信息充分性（Information Sufficiency）}：状态应包含做出最优决策所需的充分信息。信息不足会导致智能体无法区分需要不同动作的情况，从而影响学习效果。Lark的15维状态空间涵盖了TCP协议栈提供的所有关键信息，保证了信息充分性。

\textbf{维度可扩展性（Dimensional Scalability）}：状态空间的维度不应过高，否则会导致维度灾难（Curse of Dimensionality）问题。Lark的15维状态空间在现有方案中属于较高维度，但仍在可处理范围内。基于规则的决策机制避免了维度灾难问题。
...
\textbf{奖励塑形（Reward Shaping）}：适当的奖励塑形可以加速学习收敛。Lark使用对数函数对吞吐量和延迟进行变换，实现边际效用递减，有助于平衡优化目标。
...
本节详细描述Lark算法的15维观测空间设计，这是Lark区别于现有方案的核心创新之一。
...
\textbf{envType（索引1）}是环境类型标识符，用于区分事件驱动模式（值为0）和时间驱动模式（值为1）。事件驱动模式在每个TCP回调事件发生时触发决策；时间驱动模式以固定时间间隔触发决策。Lark默认采用事件驱动模式，以实现对网络状态变化的快速响应。

\textbf{simTime（索引2）}是当前仿真时间，单位为微秒。仿真时间用于：（1）计算时间间隔，如两次回调之间的时间差；（2）估算瞬时吞吐量，如$throughput = bytes / interval$；（3）跟踪ECN事件率，如统计最近1秒内的ECN事件次数。
...
\textbf{minRtt（索引10）}是历史最小往返时延，单位为微秒。minRtt被认为是网络路径的固有传播延迟（假设其他延迟分量可以忽略），用于估计带宽延迟积（BDP）和判断网络拥塞程度。RTT膨胀比定义为：
...
Lark算法的核心创新之一是多信号融合拥塞检测方法。该方法综合分析多种拥塞信号，采用分层优先级机制和频率过滤策略，准确判断网络拥塞状态并量化拥塞严重程度。
...
当ecnState为ECN\_CE\_RCVD（收到CE标记）或ECN\_ECE\_RCVD（收到ECE回显）时，判定发生ECN拥塞。ECN拥塞采用较温和的响应策略（保留因子0.85），因为ECN是早期拥塞信号，网络尚未发生实际丢包。

\subsection{频率过滤策略}

为了避免对瞬态扰动的过度响应，Lark采用频率过滤策略，将以下信号排除在拥塞触发条件之外：

\textbf{（1）单次ECN事件}：单次ECN标记可能是由于队列瞬时波动导致，不能作为持续性拥塞的判断依据。Lark通过跟踪ECN事件率，只有当事件率超过阈值时才触发拥塞响应。

\textbf{（2）ECN回显状态}：ecnState为ECN\_ECE\_RCVD表示收到了接收端的ECN回显，但此时发送端可能已经响应了该拥塞通知。为避免重复响应，不将其作为独立的拥塞触发条件。

\textbf{（3）拥塞窗口缩减状态}：caState为CA\_CWR表示发送端正在响应ECN标记并缩减窗口。此时拥塞响应已经在进行中，不需要再次触发。

\textbf{（4）恢复状态}：caState为CA\_RECOVERY表示正处于快速恢复过程中。快速恢复是对丢包的响应机制，不应重复触发拥塞响应。

\textbf{（5）RTT膨胀}：虽然RTT膨胀是拥塞的间接指示，但RTT测量易受多种因素干扰，单独依靠RTT膨胀判断拥塞可能导致误判。Lark将RTT膨胀用于自适应参数调优，而非直接用于拥塞触发。
...
ECN标记（ECN） & 0.85 & ECN是早期信号，保留较大窗口维持吞吐量 \\
...
根据时序关系和严重程度，Lark设计了三级差异化响应：ECN标记采用最温和的响应（保留因子0.85），丢包采用较激进的响应（保留因子0.70），超时采用最激进的响应（保留因子0.50）。这种阶梯式的响应力度设计确保了对不同严重程度拥塞的精确匹配。

\textbf{（2）信号可靠性分析}

丢包是最可靠的拥塞信号，几乎不会出现误报（False Positive）。ECN标记的可靠性取决于网络设备的配置，如果阈值设置不当，可能出现误报。基于可靠性考虑，对丢包信号的响应应更加坚决（较低的保留因子），对ECN信号的响应可以更加保守（较高的保留因子）。
...
除了拥塞窗口，拥塞响应还需要更新慢启动阈值。Lark采用以下策略：

\begin{equation}
new\_ssthresh = \max(new\_cwnd, BDP)
\end{equation}

这个设计的考虑是：（1）新的ssthresh应不小于新的cwnd，否则会立即进入慢启动阶段，导致窗口增长过快；（2）ssthresh设为BDP的估计值，允许在拥塞缓解后快速恢复到网络的带宽延迟积水平。

此外，Lark引入了连续缩减保护机制（Consecutive Decrease Protection）。在实际网络中，连续的拥塞事件可能导致窗口持续缩减直至极小值，严重影响吞吐量。Lark设置最大连续缩减次数为3次：当连续拥塞响应超过3次时，保持当前窗口不再缩减，等待窗口正常增长后重置计数器。这一机制有效防止了窗口的过度收缩，保障了最低传输性能。
...
\ElsIf{$type = ECN$}
    \State $\rho \leftarrow 0.85$ \Comment{ECN：温和响应}
\Else
...
\State \Comment{计算新慢启动阈值：取新窗口与BDP的较大值}
...
\begin{equation}
target\_cwnd = \max(\alpha \times BDP, cwnd) + \gamma \times MSS
\end{equation}

$\alpha$的取值决定了窗口增长的激进程度：较大的$\alpha$导致更激进的窗口增长，能够更快地利用可用带宽，但也更容易导致拥塞；较小的$\alpha$导致更保守的窗口增长，网络更稳定，但带宽利用效率较低。

\subsection{多因素调整机制}

Lark根据以下因素动态调整$\alpha$：

\textbf{（1）基于RTT膨胀比的三级梯度调整}
...
调整规则采用三级梯度响应：
\begin{itemize}
    \item 当$inflation\_ratio < 1.3$时，排队延迟极小，大幅增大$\alpha$：$\alpha \leftarrow \alpha + 0.03$
    \item 当$1.3 \leq inflation\_ratio < 2.0$时，排队延迟适中，温和增大$\alpha$：$\alpha \leftarrow \alpha + 0.01$
    \item 当$inflation\_ratio > 3.0$时，排队延迟过大，减小$\alpha$：$\alpha \leftarrow \alpha - 0.02$
\end{itemize}

相比简单的二级阈值判断，三级梯度响应机制能够更精细地适配不同程度的网络排队状态，在低排队环境中快速提升带宽利用率，在高排队环境中及时降低发送激进度。
...
其中$\eta = 0.1$为EMA平滑因子。根据奖励趋势调整$\alpha$：
\begin{itemize}
    \item 当$R_{ema} > 2.0$时，表明当前策略效果良好，增大$\alpha$：$\alpha \leftarrow \alpha + 0.01$
    \item 当$R_{ema} < -5.0$时，表明当前策略效果不佳，减小$\alpha$：$\alpha \leftarrow \alpha - 0.01$
\end{itemize}
...
当窗口连续增长超过5次时，表明网络状况持续良好，给予稳定增长奖励：
\begin{equation}
\alpha \leftarrow \alpha + 0.02 \quad \text{如果连续增长次数} > 5
\end{equation}
...
\begin{equation}
\alpha \in [\alpha_{min}, \alpha_{max}] = [1.05, 1.50]
\end{equation}

$\alpha$的默认初始值为1.20。下界1.05保证了即使在最保守的情况下，目标窗口仍略大于BDP，避免带宽利用不足；上界1.50限制了最激进的增长率，防止窗口过快膨胀导致拥塞。这个设计允许$\alpha$在较大范围内动态调整，以适应不同的网络环境。
...
\Ensure 更新后的$\alpha$
\State \Comment{因素1：基于RTT膨胀比的三级梯度调整}
\State $inflation\_ratio \leftarrow obs.lastRtt / obs.minRtt$
\If{$inflation\_ratio < 1.3$}
    \State $\alpha \leftarrow \alpha + 0.03$ \Comment{排队极小，大幅增大}
    \State $history.consecutive\_growth \leftarrow history.consecutive\_growth + 1$
\ElsIf{$inflation\_ratio < 2.0$}
    \State $\alpha \leftarrow \alpha + 0.01$ \Comment{排队适中，温和增大}
\ElsIf{$inflation\_ratio > 3.0$}
    \State $\alpha \leftarrow \alpha - 0.02$ \Comment{排队过大，减小}
    \State $history.consecutive\_growth \leftarrow 0$
\EndIf
\State \Comment{因素2：基于奖励信号EMA的趋势调整}
\State $R_{ema} \leftarrow 0.9 \times R_{ema} + 0.1 \times reward$
\If{$R_{ema} > 2.0$}
    \State $\alpha \leftarrow \alpha + 0.01$ \Comment{策略效果良好}
\ElsIf{$R_{ema} < -5.0$}
    \State $\alpha \leftarrow \alpha - 0.01$ \Comment{策略效果不佳}
\EndIf
\State \Comment{因素3：基于连续稳定增长的奖励}
\If{$history.consecutive\_growth > 5$}
    \State $\alpha \leftarrow \alpha + 0.02$
\EndIf
\State \Comment{应用边界约束}
\State $\alpha \leftarrow \max(\min(\alpha, 1.50), 1.05)$
\State \Return $\alpha$
\end{algorithmic}
\end{algorithm}
...
其中，$\alpha$是乘性增加因子（通过自适应调优动态调整，基准值1.20），$\gamma = 2.0$是加性增加因子，表示每个ACK事件增加2个MSS的窗口增量。较大的$\gamma$值使得Lark在非拥塞阶段能够更快速地增加窗口，充分利用可用带宽。
...
调整规则：
\begin{itemize}
    \item 当$utilization < 0.7$时，额外增加$1 \times MSS$
    \item 当$utilization < 0.4$时，额外增加$2 \times MSS$（累计$3 \times MSS$）
\end{itemize}

这个设计确保了在管道未充分利用时能够快速增加窗口，应对应用层突发数据到达或接收窗口临时限制解除等场景。
...
\begin{equation}
cwnd \in [4 \times MSS, \max(10 \times BDP, 200 \times MSS)]
\end{equation}

下界$4 \times MSS$是TCP的最小窗口大小，保证基本的传输能力。上界设为BDP的10倍或200个MSS的较大值，提供充裕的窗口增长空间以充分利用管道容量，同时避免窗口无限增长导致网络不稳定。
...
\caption{融合控制策略算法}
\begin{algorithmic}[1]
\Require 观测状态$obs$，自适应参数$\alpha$，$\gamma = 2.0$，BDP估计$bdp$
\Ensure 新窗口$new\_cwnd$
\State \Comment{重置连续缩减计数器}
\State $consecutive\_decreases \leftarrow 0$
...
\If{$obs.cwnd < obs.ssthresh$ \textbf{and} $in\_slow\_start$}
    \State \Comment{慢启动阶段}
    \State $target \leftarrow 2 \times bdp$
...
\Else
    \State \Comment{拥塞避免阶段}
    \State $target\_rate \leftarrow \alpha \times bdp$
    \State $\gamma\_bytes \leftarrow \gamma \times obs.segSize$
    \State $new\_cwnd \leftarrow \lfloor \max(target\_rate, obs.cwnd) + \gamma\_bytes \rfloor$
    \State \Comment{管道利用率感知加速}
    \If{$utilization < 0.4$}
        \State $new\_cwnd \leftarrow new\_cwnd + 3 \times obs.segSize$
    \ElsIf{$utilization < 0.7$}
        \State $new\_cwnd \leftarrow new\_cwnd + 1 \times obs.segSize$
    \EndIf
\EndIf
\State \Comment{应用边界约束}
\State $min\_cwnd \leftarrow 4 \times obs.segSize$
\State $max\_cwnd \leftarrow \max(10 \times bdp, 200 \times obs.segSize)$
\State $new\_cwnd \leftarrow \max(\min(new\_cwnd, max\_cwnd), min\_cwnd)$
\State \Return $new\_cwnd$
\end{algorithmic}
\end{algorithm}
...
\textbf{（1）历史度量缓冲区}：带宽估计采用窗口化最大值过滤，维护一个容量为20个样本点的投递率（Delivery Rate）滑动窗口队列，取窗口内最大值作为瓶颈带宽估计。RTT跟踪维护一个容量为40个样本点的最近RTT队列，同时维护全局最小RTT作为传播延迟估计。BDP由最大带宽与最小RTT的乘积得到。

\textbf{（2）ECN事件跟踪数据}：ECN事件跟踪模块维护ECN事件计数器，记录丢包事件计数和ECN标记事件计数，用于统计分析拥塞事件的分布特征。

\textbf{（3）性能指标数据}：性能指标模块维护吞吐量的指数移动平均值（EMA，平滑因子0.1），用于跟踪流级别的性能变化趋势。

\textbf{（4）自适应参数数据}：自适应参数模块维护乘性增加因子$\alpha$（取值范围$[1.05, 1.50]$，默认值$1.20$）、加性增加因子$\gamma$（默认值$2.0$）、连续缩减计数器（用于连续缩减保护）、连续增长计数器（用于稳定增长奖励）和慢启动阶段标记。
...
\State $agent.\alpha_{default} \leftarrow 1.20$
...
Lark算法采用高效的数据结构支持其核心功能。\textbf{循环队列（Circular Buffer）}用于存储历史度量值，具有固定内存占用、$O(1)$时间复杂度的插入和删除操作、自动保持时间局部性等优势。\textbf{ECN事件跟踪器}用于计算ECN事件率，维护时间戳序列支持快速查询。\textbf{性能指标聚合器}负责维护流级别的性能统计信息。
...
\Function{GetOrCreateFlowState}{$manager$, $socket\_uuid$}
    \If{$socket\_uuid \notin manager.flow\_states$}
        ...
        \State $new\_state.\alpha \leftarrow 1.20$
...
\textbf{（3）查找表优化}：窗口保留因子使用查找表实现，避免条件分支带来的CPU流水线停顿。

\textbf{（4）批量状态更新}：将多次更新操作合并为一次，减少函数调用开销和内存访问次数。

\textbf{（5）缓存友好设计}：将频繁访问的字段放置在相邻的内存位置，提高CPU缓存的命中率。

\subsection{多线程并行化架构}

为了提高仿真效率，Lark系统采用了多线程并行化仿真架构。拓扑分区策略将网络拓扑划分为多个逻辑分区，遵循负载均衡、最小化跨分区通信和保持拓扑局部性原则。并行事件调度采用保守同步协议，通过空消息机制确保因果一致性。

\begin{algorithm}[htbp]
\caption{保守同步并行仿真}
\begin{algorithmic}[1]
\Require 分区集合 $partitions$，终止时间 $end\_time$
\For{each $partition$ $p$ in $partitions$}
    \State $p.lvt \leftarrow 0$
    \State $p.lookahead \leftarrow$ ComputeLookahead($p$)
\EndFor
\While{$\exists p : p.lvt < end\_time$}
    \State \textbf{并行执行} $\forall p \in partitions$:
    \State \quad $safe\_time \leftarrow$ ComputeSafeTime($p$)
    \While{$p.event\_queue$.PeekTime() $\leq safe\_time$}
        \State \quad\quad $event \leftarrow p.event\_queue$.Dequeue()
        \State \quad\quad ProcessEvent($p$, $event$)
    \EndWhile
    \State \quad SendNullMessages($p$, $p.lvt + p.lookahead$)
    \State \textbf{结束并行执行}
    \State Barrier()
\EndWhile
\end{algorithmic}
...
实验表明，在8核CPU上，多线程并行化可以获得3--5倍的仿真加速，显著提高了大规模网络仿真的效率。
...
（2）15维观测空间：涵盖连接标识、时间信息、窗口状态、传输指标、延迟测量、回调类型和拥塞控制状态等多个维度，为强化学习智能体提供完整的环境感知能力。

（3）多信号融合拥塞检测：采用分层优先级机制和频率过滤策略，综合分析丢包、ECN、拥塞状态等多种信号，准确判断拥塞状态并量化严重程度。

（4）差异化拥塞响应：针对不同拥塞类型采用不同的窗口保留因子（丢包0.70、ECN 0.85、超时0.50），并引入连续缩减保护机制（最多3次连续缩减后保持窗口），实现对拥塞信号的精准响应。

（5）自适应参数调优：根据RTT膨胀比的三级梯度响应、奖励信号EMA趋势和连续稳定增长动态调整乘性增加因子$\alpha \in [1.05, 1.50]$（基准值1.20），实现控制策略对网络环境的实时适配。

（6）融合控制策略：按公式$V_t = \max(\alpha \times BDP, W) + \gamma \times MSS$（$\gamma = 2.0$）将基于BDP窗口化最大值过滤估计的速率控制与基于当前窗口的保守控制相结合，在慢启动阶段以$2 \times BDP$为目标，在拥塞避免阶段引入管道利用率感知的加速增长机制。
