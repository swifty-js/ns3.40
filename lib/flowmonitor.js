...
export class ScenarioResult {
  ...
  /** @returns {number} Summed forward goodput in Mbps. */
  get totalThroughputMbps() {
    return sum(this.forwardFlows.map((flow) => flow.throughputMbps));
  }
...
export function forwardKpi(filepath) {
  ...
  for (const element of asArray(child(stats, "Flow"))) {
    ...
    const durationS =
      (parseNsTime(stringAttribute(attributes, "timeLastRxPacket")) -
        parseNsTime(stringAttribute(attributes, "timeFirstTxPacket"))) /
      1e9;
    if (durationS > 0) goodputs.push((rxBytes * 8) / durationS / 1e6);
    if (rxPackets > 0) {
  ...
  const total = sum(goodputs);
  const squares = sum(goodputs.map((value) => value * value));
  const jain = nflow > 0 && squares > 0 ? (total * total) / (nflow * squares) : 0;
