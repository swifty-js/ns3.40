...
private:
/\*\*
_ Private method doxygen is also recommended
_/
...
- If a function's or a method's parameter is definitely unused,
  ...
  void
  UanMacAloha::RxPacketGood(Ptr<Packet> pkt, double /_sinr_/, UanTxMode txMode)
  {
  UanHeaderCommon header;
  pkt->RemoveHeader(header);
  ...
...
- Consider caching frequently-used results (especially expensive calculations,
  ...
  for (uint8*t i = 0; i < NUM_VALUES; i++)
  {
  array1[i] = (std::pow(2, i) * std::sin(theta)) + std::cos(theta);
  array2[i] = (std::pow(2, i) \_ std::cos(theta)) + std::sin(theta);
  }
