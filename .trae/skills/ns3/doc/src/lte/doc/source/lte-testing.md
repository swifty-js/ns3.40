...
In more specific, the tests verify the _timing_ and the _content_ of each
...
The motivation behind the _"teleport"_ between the predefined spots is to
...
places the nodes on a straight line and instruct the UE to _"jump"_ in a similar
...
values of time-to-trigger: _short_ (shorter than report interval), _long_
(shorter than the filter measurement period of 200 ms), and _super_ (longer than
...
.. math::
\rho*{R,i} = \frac{R_i}{\sum*{j=1}^N R_j}
...
.. math::
\rho*{T,i} = \frac{T_i}{\sum*{j=1}^N T_j}
...
.. math::
\frac{T*i}{\sum*{j=1}^N T*j} &= \frac{c R_i}{\sum*{j=1}^N c R*j} \\
&= \frac{c R_i}{c \sum*{j=1}^N R*j} \\
...
\mathrm{T\_{test-1}} = 0.0 \times 1 + 0.926 \times 2 + 0.074 \times 3 = 2.074

\mathrm{T\_{test-2}} = 0.0 \times 1 + 0.752 \times 2 + 0.248 \times 3 = 2.248
...
\mathrm{Thr*{test-i}} = \frac{TTI*{NUM}}{T*{test-i}} TB*{size} = \left\{ \begin{array}{lll} \dfrac{1000}{2.074}66 = 31822\mbox{ bps} & \mbox{ for test-1} \\ & \\ \dfrac{1000}{2.248}472 = 209964\mbox{ bps} & \mbox{ for test-2}\end{array} \right.
...
time-consuming test suite in ns-3. The test cases run with _some_ combination of
