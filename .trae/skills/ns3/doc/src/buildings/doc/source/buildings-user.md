...
method of the `MobilityBuildingInfo`class. In particular, it calls the`MakeMobilityModelConsistent`method, which goes through the lists of
...
consistent for a moving node whenever the`IsInside`method of`MobilityBuildingInfo`` class is called.
