...
The acronym _Yans_ derives from this paper title. The SpectrumWifiPhy
...
_Note that there is already such a name inside the wave module
...
that name, so no ambiguity in using the name for 802.11 amendments._
...
layer, and allows other objects to hook as _listeners_ to monitor PHY
...
preamble and the PHY is put into the CCA*BUSY state. Currently, there is only a
...
_Comparison of table-based OFDM Error Model with TGax results._
...
_YANS and NIST error model comparison with TGn results_
...
slot boundary, which basically occurs following SIFS + AIFSN _ slotTime of idle
medium after the last busy medium that was the result of a reception of a frame
with a correct FCS or following EIFS - DIFS + AIFSN _ slotTime + SIFS of idle
...
_FrameExchangeManager hierarchy_
...
an A-MPDU can be sent with the _Normal Ack or Implicit Block Ack Request_ policy, in which
case the receiver replies with a Normal Ack or a Block Ack depending on whether the A-MPDU
contains a single MPDU or multiple MPDUs, or with the _Block Ack_ policy, in which case
...
- `UseExplicitBar`: used to determine the ack policy to use when a response is needed from
  ...
  true, the _Block Ack_ policy is used. Otherwise, the _Implicit Block Ack Request_ policy is used.
...
Consider node _A_ sending a unicast packet to node _B_.
When _B_ successfully receives the packet sent from _A_,
_B_ records the SNR of the received packet into a `ns3::SnrTag`
and adds the tag to an ACK back to _A_.
By doing this, _A_ is able to learn the SNR of the packet sent to _B_
using an out-of-band mechanism (thus the name 'ideal').
_A_ then uses the SNR to select a transmission mode based
