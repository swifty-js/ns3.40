...
idiom used throughout the code: the _ns-3 callback_. This chapter provides some
...
class A {
public:
void ReceiveInput( /_ parameters _/ );
...
}
...
B::DoSomething()
{
// Tell a*instance that something happened
a_instance->ReceiveInput( /* parameters \_/ );
...
}
...
_callback_. The ultimate goal is to allow one piece of code to call a function
...
and _call back_ to let you know it completed. It calls back by following the
...
module which will _call back_ using a method and an object pointer. The
...
One might ask at this time, _what's the point_? The called module will have to
...
calls the process of allowing a client to specify one of the parameters _"binding"_.
...
What we want to do is to _bind_ the `Ptr<PcapFileWriter> file` to the
