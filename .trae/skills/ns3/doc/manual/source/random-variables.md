...
- in _ns-3.3_ and earlier, |ns3| simulations used a random seed by default; this
  marks a change in policy starting with _ns-3.4_.

- in _ns-3.14_ and earlier, |ns3| simulations used a different wrapper class
  called :cpp:class:`ns3::RandomVariable`. As of _ns-3.15_, this class has been
...
The length of this sequence is called the _cycle length_
or _period_, after which the RNG will repeat itself.
This sequence can
be partitioned into disjoint _streams_. A stream of a
...
uncorrelated _substreams_. The underlying RNG hopefully
...
period (_i.e._, the number of random numbers before overlap) of
...
substream capability provided by the RNG implementation. _Therefore, use the
substream capability to produce multiple independent runs of the same
simulation._ In other words, the more statistically rigorous way to configure
