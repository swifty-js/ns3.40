...
to its specification is generically called _verification_. The process of
...
Every model has a _target system_ that it is attempting to simulate. The
...
called _model qualification_ in the literature. In the case of a TCP
...
implementation to agree with the abstract model is called _model
verification_ in the literature.
...
qualification process is called _model validation_ in the literature. In the
...
The _domain of applicability_ is a set of prescribed conditions for which
the model has been tested, compared against reality to the extent possible, and
judged suitable for use. The _range of accuracy_ is an agreement between
...
system is integrated, it is called generically a _regression_.
Originally the
...
A _local regression_ is one in which a change affects the changed component
...
A _remote regression_ is one in which a change to one component breaks
...
An _unmasked regression_ is one that creates a situation where a previously
...
A _performance regression_ is one that causes the performance requirements
