...
using Doxygen\_, to generate cross-linked web pages.
Both of these are important: the Sphinx chapters explain the _why_
...
We use Sphinx* to generate expository chapters describing
...
`Show Source <_sources/documentation.txt>`* link in the sidebar
...
Sphinx has to know _where_ your new chapter should appear. In most
...
The Sphinx documentation* and tutorial* are pretty good. We won't duplicate
...
We use Doxygen* to generate browsable* API documentation. Doxygen
...
indicating _where_ each file, class, method, and variable is used.
...
documentation sections for _all_ items, even if they do not have
...
(This snippet has _a lot_ of lines suppressed!)
...
warnings.) As you can see, at this writing we have _a lot_ of
...
typedefs, _etc._ won't produce documentation when you build with
...
As for Sphinx, the Doxygen docs* and reference* are pretty good.
...
- Did you know `typedefs` can have formal arguments? This enables
  ...
  - \param ale The size of a pint of ale, in Imperial ounces.
    _/
    typedef void (_ BarCallback)(const int ale);
...
- Dummy destructor and DoDispose::
  ...
  /\*_ Dummy destructor, see DoDispose. _/
  ~MyClass();
