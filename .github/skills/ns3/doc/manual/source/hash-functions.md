...
is murmur3*, chosen because it has good hash function properties and
...
The default hash function is murmur3*. FNV1a* is also available. To specify
...
typedef uint32*t (\_Hash32Function_ptr) (const char*, const size*t);
typedef uint64_t (\_Hash64Function_ptr) (const char*, const size_t);
