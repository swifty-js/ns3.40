...
the next step is to configure the build using the _CMake_ build system. The below commands
make use of a Python wrapper around CMake, called `ns3`, that simplifies the command-line
syntax, resembling _Waf_ syntax. There are several options to control the build, but enabling
