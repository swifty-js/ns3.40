...
- |ns3| installation requirements vary from release to release, and also as underlying operating
  systems evolve. This document is version controlled, so if you are using the _ns-3.X_ release,
  try to read the _ns-3.X_ version of this document. Even with this guidance, you may encounter
...
As of _ns-3.37_, |ns3| uses `cppyy` to generate runtime Python
...
standardized such package distribution, favoring instead to recommend a _source download_ without
