...
In order to _not_ install IPv6 along with IPv4, it is possible to use
...
In IPv6 there is a clear distinction between _routers_ and _hosts_. As one might expect,
...
In |ns3| a node is configured to be an _host_ by default. There are two main ways to change
this behaviour:
...
This will enable forwarding on the node _0_ and will setup a default route in
...
Remember that addresses in IPv6 are _global_ by definition. When using IPv6 with an emulation
...
IPv6 standard (RFC 3849) defines the `2001:DB8::/32` address class for the _documentation_.
