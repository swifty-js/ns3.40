...
- support for kernel implementations via `Direct Code Execution (DCE) <https://www.nsnam.org/overview/projects/direct-code-execution/>`\_\_
...
data senders usually binds automatically after a _Connect()_ over a random
...
other peer invoked _Connect()_) the application will be signaled with the
callback _NotifyConnectionRequest_ (set in _SetAcceptCallback()_ beforehand).
...
created to handle the incoming data/connection, in the state SYN*RCVD. Please
...
created socket is provided in the Callback \_NotifyNewConnectionCreated* (set
beforehand in _SetAcceptCallback_), and should be used to connect new
...
data to transmit, please refer to the _Send()_ / _Close()_ combination of
API.
...
is present (i.e. _GetRxAvailable_ returns 0) an empty packet is returned.
Set the callback _RecvCallback_ through _SetRecvCallback()_ in order to have
...
TCP then will be in the SYN*SENT state. If a SYN-ACK is received, the TCP will
...
the application calls _Close()_ with unread data in its rx buffer, the socket
will send a reset. If the socket is in the state SYN*SENT, CLOSING, LISTEN,
FIN_WAIT_2, or LAST_ACK, after that call the application will be notified with
\_NotifyNormalClose()*. In other cases, the notification is delayed
(see _NotifyNormalClose()_).
...
_NotifyConnectionSucceeded_: _SetConnectCallback_, 1st argument
...
_NotifyConnectionFailed_: _SetConnectCallback_, 2nd argument
Called after the SYN retransmission count goes to 0. SYN packet is lost
multiple times, and the socket gives up.

_NotifyNormalClose_: _SetCloseCallbacks_, 1st argument
...
- The application tries to _Connect()_ over an already connected socket
...
_NotifyErrorClose_: _SetCloseCallbacks_, 2nd argument
Invoked when we send an RST segment (for whatever reason) or we reached the
maximum amount of data retries.

_NotifyConnectionRequest_: _SetAcceptCallback_, 1st argument
...
_NotifyNewConnectionCreated_: _SetAcceptCallback_, 2nd argument
Invoked when from SYN*RCVD the socket passes to ESTABLISHED, and after setting
up the congestion control, the sequence numbers, and processing the incoming
ACK. If there is some space in the buffer, \_NotifySend* is called shortly
after this callback. The Socket pointer, passed with this callback, is the
newly created socket, after a Fork().

_NotifyDataSent_: _SetDataSentCallback_
...
_NotifySend_: _SetSendCallback_
...
_NotifyDataRecv_: _SetRecvCallback_
...
Smoothed one way delay = 7/8 _ Old Smoothed one way delay + 1/8 _ one way delay
...
- _Receiver functionality:_ If CE is observed in the IP header of an incoming
...
- _Sender functionality:_ The sender makes use of the modified receiver
...
- _g_ is the estimation gain (between 0 and 1)
- _F_ is the fraction of packets marked in current RTT.
