...
- a set of mobility models which are used to track and maintain the _current_ cartesian position and speed of an object.
...
is typically aggregated to an`ns3::Node`object and queried using`GetObject<MobilityModel> ()`. The base class `ns3::MobilityModel``
...
$ns at $time $node set X* x1
$ns at $time $node set Y* Y1
$ns at $time $node set Z\* Z1
...
int64*t streamIndex = /\_some positive integer */
MobilityHelper mobility;
... (configure mobility)
mobility.Install(wifiStaNodes);
