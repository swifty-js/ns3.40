...
network services in the kernel. A _socket_ is an abstraction, like
...
understand the differences between them. The first is a _native_
...
to understand that the semantics are _not_ the exact same as
...
- there is no notion of synchronous API or a _blocking_ API;
...
- many calls use :cpp:class:`ns3::Packet` class to transfer data
  ...
  pass _Packets_ across a stream socket API, but think
...
so we adopt the following model. There are certain _factory_
...
These models include _blocking sockets_, _signal-based I/O_,
and _non-blocking sockets_ with polling. In ns-3, we make use
of the callback mechanisms to support a fourth mode, which is
analogous to POSIX _asynchronous I/O_.
...
of the Type of Service field (renamed as Differentiated Services field by RFC 2474) of the IPv4 header of the packets sent through that socket, as detailed
